...
def ai_enrich(D):
    ...
    ctx = {
        ...
        "游戏周年月份": [{"游戏": g["cn"], "周年月份": g["anniversary_month"]}
                      for g in CFG["games"] if g.get("anniversary_month")],
        "我方游戏": MY, "我方本周": D["my"],
    }
    prompt = (
        "你是日本二次元手游市场分析师。请严格保持客观、准确、中性：不夸大不情绪化(避免'破产''退坑''炎上'等词)；当评论提及'周年/アニバーサリー'时, 结合'游戏周年月份'判断: 若周期时间在该游戏周年月份前后1个月内则为'周年', 否则为'半周年'并如实标注；指代明确避免歧义(如'主角/指挥官'要写清具体对象)；避免日语直译词(如'天井'应写为'保底', '炎上'应写为'差评爆发', 'ガチャ'应写为'抽卡')。以下是本周日区竞品监测数据(JSON)。请输出严格的JSON(不要markdown代码块), 字段:\n"
        '{"headline": "本周一句话焦点摘要(40字内, 点出最重要的1-2个市场事件)",\n'
        ' "update_cn": {"游戏名": "该版本更新内容的中文一句话概括(30字内, 提取新角色/新活动名)"},\n'
        ' "alert_cn": {"游戏名": "差评原因的中文一句话概括(基于差评样本, 25字内)"},\n'
